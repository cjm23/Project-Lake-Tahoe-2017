...
all_data <- read.csv("E:/SNPLMA3/LTW_PM25_emissions_scenv3.csv")

all_data$day <- as.numeric(all_data$day)
all_data$PM2.5.Flaming <- as.numeric(all_data$PM2.5.Flaming)
all_data$PN2.5.Smoldering <- as.numeric(all_data$PM2.5.Smoldering)
all_data$Year <- as.numeric(all_data$Year)
all_data$Climate <- as.factor(all_data$Climate)
scen_clim <- levels(all_data$Climate)
timesteps <- 1:100
t <- timesteps[1]
  ...
  all_data3 <- NULL
    for(s in scen_dir){
      ...
      for(r in scen_rep){
        ...
        for(t in timesteps){
          all_data3 <- NULL
          fe3 <- subset(all_data2, Climate == cl)
          fe1 <- subset(fe3, Scenario == s)  
        ...
        for(i in dfire){
          ...
          all_data4 <- cbind(all_data3, cl, s, r, t)
        }
...
colnames(all_data6) <- c("Day", "TotPM","Climate", "Scenario", "Replicate", "Year")  
...
all_data6$Climate <- as.factor(all_data6$Climate)

all_data5 <- all_data6 %>%                       ##sum across identical days to get total pm2.5 for a calendar day
  group_by(Climate, Scenario, Replicate, Year, Day) %>%
  summarise(Total_PM2.5 = sum(TotPM))

write.csv(all_data5, "F:/SNPLMA3/Round2_LTW_emissions_byday.csv")
...
r2_s1_5 <- read.csv("F:/SNPLMA3/R2S1-5_annual_cumulative_emissions.csv")

r2_ace <- ggplot(data = r2_s1_5, mapping = aes(x = Year, y = MeanCumPM2.5, linetype = Scenario, fill = Scenario)) +
  geom_line() +
  geom_ribbon(aes(ymax = MeanCumPM2.5 + SDCumPM2.5, ymin = MeanCumPM2.5 - SDCumPM2.5), alpha = 0.7) +
  scale_y_continuous(labels = comma) +
  scale_fill_manual(values=cbPalette) +
...
pdf("F:/SNPLMA3/r2s1-5_cumulative_emissionsv2.pdf",         # File name
    width = 5, 
    height = 5, # Width and height in inches
)
